...
sudo ufw allow 49160:49660/udp   # диапазон релея, см. docker/turnserver.conf
```
...
Проверить снаружи: в CRM открыть телефонный коннектор → «Проверить релей».

- Порты открывает `deploy.sh` (шаг 6) — если он выполнялся из-под root. Иначе
  он это скажет, и останется выполнить `sudo bash deploy/open-turn-ports.sh`.
- Если сервер за NAT (облако с 1:1 NAT — AWS, Yandex Cloud), добавь в `.env`
  `TURN_EXTRA_ARGS=--external-ip=<белый IP>` и `docker compose restart turn`.
...
8. Откроет в фаерволе порты релея звонков (если TURN не выключен в `.env`).
