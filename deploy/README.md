...
├── open-turn-ports.sh                       ← порты релея звонков в фаерволе
├── turn-detect-nat.sh                       ← внешний адрес релея, если сервер за NAT
├── turn-ports.service                       ← systemd-юнит: порты после перезагрузки
...
Встроенному релею звонков (TURN) нужны свои порты: он работает в host-сети, а
такие контейнеры, в отличие от опубликованных портов, правила ufw **не**
обходят. Отдельно делать ничего не нужно — их открывает `deploy.sh` на шаге 6.

Если ставишь FARA без прокси-стека, открой их сам (скрипт идемпотентный, сам
разбирается с ufw / firewalld / голым iptables и берёт диапазон портов из
`docker/turnserver.conf` — так фаервол не разъедется с тем, что реально
раздаёт релей):
...
Руками это те же три правила:

```bash
sudo ufw allow 3478/udp
sudo ufw allow 3478/tcp
sudo ufw allow 49160:49259/udp   # диапазон релея, см. docker/turnserver.conf
```
...
### 4a. Релей звонков (поднялся сам — проверь)

Релей (coturn) входит в поставку и стартует вместе с остальными сервисами.
Настраивать в `.env` ничего не нужно: секрет генерируется при первом запуске,
адрес берётся из `site_url` (шаг 3). Без релея не соединятся звонки у тех, кто
сидит за симметричным NAT или в офисе с закрытым наружу UDP.

```bash
docker compose ps turn          # должен быть running (healthy)
ss -lunp | grep 3478            # слушает порт хоста
```

Проверить из CRM: «Настройки» → «Релей звонков (TURN)» → «Проверить релей».
Одна кнопка показывает сразу все состояния: выдаётся ли релей браузеру,
отвечает ли он, принят ли секрет, публичный ли адрес он раздаёт, поднимается ли
через него соединение из этого браузера и пустит ли он трафик до АТС. К каждой
проблеме там же приложена команда, которой она лечится.

- Всё нужное делает `deploy.sh` (шаг 6), если запускался из-под root: порты и
  внешний адрес. Иначе он это скажет, и останется выполнить две команды —
  `sudo bash deploy/turn-detect-nat.sh` и `sudo bash deploy/open-turn-ports.sh`.
- **NAT** (облако с 1:1 NAT — AWS, Yandex Cloud) определяется автоматически:
  скрипт сверяет внешний адрес с адресами на интерфейсах и, если они разные,
  дописывает в `.env` `TURN_EXTRA_ARGS="--external-ip=БЕЛЫЙ/СВОЙ"`. Заданный
  руками `TURN_EXTRA_ARGS` он не трогает.
- **АТС в приватной сети** — единственное, что автоматика угадать не может:
  релею запрещено ходить в приватные диапазоны, и для АТС нужно точечное
  исключение. Добавьте к `TURN_EXTRA_ARGS` флаг `--allowed-peer-ip=<IP АТС>`
  (именно один адрес, не подсеть) и `docker compose up -d turn`.
- Параметры (порт, ttl пропусков, режим «всё через релей») меняются в
  «Системных настройках» по ключам `turn.*` — без перезапуска.
- Не нужен вовсе: `TURN__ENABLED=false` в `.env` плюс `docker compose stop turn`.
...
8. Настроит релей звонков (если TURN не выключен в `.env`): определит, стоит
   ли машина за NAT, и при необходимости пропишет ему внешний адрес, а затем
   откроет в фаерволе нужные порты.

Последний шаг — единственная причина запускать скрипт через `sudo`: фаервол
хостовый, из контейнера его не тронуть. Без root скрипт отработает как и
раньше, просто напечатает команду для портов отдельной строкой.
