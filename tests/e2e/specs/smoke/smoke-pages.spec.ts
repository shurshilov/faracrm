...
async function expectPageLoaded(page: any) {
  ...
  await expect(
    page
      .locator(
        'table, [class*="List"], [class*="Kanban"], [class*="Gantt"], [class*="Form"], [class*="Card"], [class*="mantine-"], button, input, h1, h2, h3',
      )
      .first(),
  ...
  const errorText = page.locator(
    "text=/Unhandled|Something went wrong|500|Internal Server Error/i",
  );
  ...
  for (const model of GANTT_MODELS) {
    test(`/${model} — Gantt view открывается`, async ({ page }) => {
      ...
      const viewSwitcher = page.locator(
        '[class*="ViewSwitcher"], [class*="viewSwitcher"]',
      );
