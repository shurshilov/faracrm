...
test.describe("WebSocket — new_message", () => {
  ...
  test("user2 получает new_message от admin", async ({
    adminToken,
    adminSession,
    user2WS,
    api,
  ...
  test("admin получает new_message от user2", async ({
    adminWS,
    user2Token,
    user2Session,
    api,
  ...
  test("отправитель НЕ получает своё сообщение", async ({
    adminWS,
    adminToken,
    adminSession,
    api,
  ...
  test("несколько сообщений в правильном порядке", async ({
    adminToken,
    adminSession,
    user2WS,
    api,
    ...
    await user2WS.waitFor(
      (msg) => msg.type === "new_message" && msg.message?.body === "Msg 3",
    );
    const msgs = user2WS
      .getMessages()
...
test.describe("WebSocket — edit/delete", () => {
  ...
  test("user2 получает message_edited", async ({
    adminToken,
    adminSession,
    user2WS,
    api,
  ...
  test("user2 получает message_deleted", async ({
    adminToken,
    adminSession,
    user2WS,
    api,
...
test.describe("WebSocket — presence", () => {
  ...
  test("presence:offline при отключении", async ({
    ...
    const onlineEvent = await adminws
      .waitForPresence(user3Session.user_id.id, "online", 10_000)
      .catch(() => null);
    if (!onlineEvent) {
      ...
      console.log(
        "[DIAG] presence:online not received — likely already subscribed via createChat",
      );
      await adminws.close();
      await api.deleteChat(adminSession, chat.id);
      test.skip();
...
test.describe("WebSocket — chat_created", () => {
  test("user2 получает chat_created", async ({
    user2WS,
    adminToken,
    adminSession,
    user2Session,
