...
class TestPasswordValidation:
    ...
    def test_require_uppercase_pass(self):
        from backend.base.crm.users.models.users import User
        errors = User.validate_password("Password", {"require_uppercase": True, "min_length": 1})
    ...
    def test_require_uppercase_cyrillic(self):
        from backend.base.crm.users.models.users import User
        errors = User.validate_password("Пароль123", {"require_uppercase": True, "min_length": 1})
        assert "no_uppercase" not in errors

    def test_require_lowercase_pass(self):
        from backend.base.crm.users.models.users import User
        errors = User.validate_password("pASSWORD", {"require_lowercase": True, "min_length": 1})
    ...
    def test_require_lowercase_cyrillic(self):
        from backend.base.crm.users.models.users import User
        errors = User.validate_password("пароль", {"require_lowercase": True, "min_length": 1})
        assert "no_lowercase" not in errors

    def test_require_digits_pass(self):
        from backend.base.crm.users.models.users import User
        errors = User.validate_password("pass1word", {"require_digits": True, "min_length": 1})
        assert "no_digit" not in errors

    def test_require_digits_fail(self):
        from backend.base.crm.users.models.users import User
        errors = User.validate_password("password", {"require_digits": True, "min_length": 1})
        assert "no_digit" in errors

    def test_require_special_pass(self):
        from backend.base.crm.users.models.users import User
        errors = User.validate_password("pass!", {"require_special": True, "min_length": 1})
        assert "no_special" not in errors

    def test_require_special_fail(self):
        from backend.base.crm.users.models.users import User
        errors = User.validate_password("password", {"require_special": True, "min_length": 1})
    ...
    def test_require_special_various_chars(self):
        ...
        for char in "!@#$%^&*()_+-=[]{}|;':\",./<>?`~":
            ...
            assert "no_special" not in errors, f"Failed for special char: {char}"
...
class TestPasswordHash:
    ...
    def test_hash_consistent_length(self):
        ...
        u = User.__new__(User)
        hashes = [
            u.generate_password_hash(f"pass{i}", f"salt{i}")
            for i in range(10)
        ]
...
class TestUserConstants:
    ...
    def test_constants_different(self):
        from backend.base.crm.users.models.users import ADMIN_USER_ID, SYSTEM_USER_ID
...
class TestModelFields:
    ...
    def test_user_has_required_fields(self):
        ...
        required = {"id", "name", "login", "password_hash", "password_salt", "is_admin"}
        for f in required:
