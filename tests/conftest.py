...
async def test_env(db_pool):
    ...
    # настоящая транзакция (роут create автокруда пишет запись и связи в
    # одной), а не MagicMock, который «проглотил» бы async with
    from backend.base.system.dotorm.dotorm.databases.postgres.transaction import (
        ContainerTransaction,
    )

    env.apps.db.get_transaction = lambda: ContainerTransaction(db_pool)
