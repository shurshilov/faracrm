...
class ChatConnectorEmailMixin(_Base):
    ...
    # `| None`: поле есть у всех коннекторов, заполняется только у почтовых.
    # Аннотации расширений уходят в API-схему, голый str сделал бы его
    # обязательным для каждого коннектора.
    email_username: str | None = Char(
        max_length=255,
