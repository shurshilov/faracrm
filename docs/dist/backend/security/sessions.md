...
| `use_anonymous_session` | Ничего, прав не даёт: нужное ручка читает и пишет сама через `.sudo()` после своей проверки доверия (пароль, код из письма, хеш вебхука, подпись провайдера, state OAuth) | Все `router_public`: `/signin`, регистрация, капча, вебхуки чата и оплаты, OAuth-колбэки, публичный конфиг и брендинг, каталог маркетплейса, WebSocket-роуты, публичные иконки |
