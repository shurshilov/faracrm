...
В прикладном коде права поднимают точечно через `.sudo()` на модели или записи: `await env.models.system_settings.sudo().get_value("turn.secret")`. Как в Odoo, `sudo()` поднимает права, но не меняет пользователя: автор записи (`create_user_id`) и владелец по умолчанию остаются вызывающим, а в публичной ручке — анонимным пользователем (id=4). Системный пользователь подставляется только там, где вызывающего нет: post_init, cron, вебхуки.
