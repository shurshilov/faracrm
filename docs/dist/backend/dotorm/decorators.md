...
- Вызов один на операцию, как `@api.constrains` с recordset в Odoo. `self` — пустой экземпляр модели, как у `hybridmethod` при вызове от класса, для `self.sudo().search(...)`. `records` — записываемые payload'ы: одна запись у `create` и `update`, все строки у bulk. На `create` у записи `id` пустой, на `update` выставлен.
