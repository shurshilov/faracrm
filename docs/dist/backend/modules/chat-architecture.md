...
Кто онлайн — состояние ПРОЦЕССА (`_connections` живёт в памяти воркера), поэтому объявление присутствия идёт по шине, а отвечает на него каждый воркер за своих подключённых:

1. Юзер добавляется в `_connections[user_id]` и присылает `subscribe_all` со своими чатами.
2. CM публикует в pub/sub `presence_hello` — `{user_id, chat_ids}`.
3. Каждый воркер шлёт своим онлайн-собеседникам по этим чатам `{type: "presence_update", add: [user_id]}`, а вошедшему — встречный кадр со списком СВОИХ онлайн-пиров.
4. При disconnect последнего соединения публикуется `presence_bye`, и те же воркеры рассылают `{remove: [user_id]}`.

Локальным снимком получателей тут не обойтись: `_chat_subscriptions` знает только соединения своего процесса, а воркеров несколько (`uvicorn --workers`).
...
!!! info "Молчащие соединения"
    Мобильная сеть рвёт TCP без close-кадра (сон вкладки, NAT оператора, WiFi↔LTE) — такой сокет выглядит живым для обеих сторон. Клиент шлёт `ping` раз в 30 с и сам переподключается, если перестали приходить `pong`; сервер раз в минуту выбрасывает соединения, молчащие дольше `WS_IDLE_TIMEOUT_SECONDS` (120 с), — иначе ушедший навсегда остался бы «в сети» для остальных.

!!! warning "Спросить «юзер X онлайн?» нельзя"
    Presence — поток событий, а не запрос. Каждый воркер знает только своих подключённых, и глобального ответа на вопрос «сейчас юзер X онлайн?» ни у кого нет: картинка собирается у КЛИЕНТА из приходящих `presence_update`. Поэтому серверный код не должен принимать решений по «онлайну» — для проверки достижимости собеседника используется отдельный ack (см. [Звонки](calls.md)).
