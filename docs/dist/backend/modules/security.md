...
Точечно права поднимает `.sudo()` (`await Model.sudo().search(...)`, `await record.sudo().update(...)`): как в Odoo, на время вызова поднимается флаг, а сессия в контексте не меняется — автор записей, владелец по умолчанию и язык остаются вызывающего, а не System. Системная и анонимная сессии отдают `get_lang() = "en"`.

Решение «можно всё» принимается в одном месте — `AccessChecker.is_full_access(session)`: в ядре это `is_sudo()` (флаг `.sudo()` или `SystemSession` в контексте), FARA добавляет суперпользователя. ORM спрашивает его перед ACL, правилами и полями; разборщик фильтров пускает `private`-поля только под `is_sudo()` — суперпользователю их не дают, иначе по `like` можно подобрать хеш посимвольно.
