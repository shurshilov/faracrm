...
FARA использует домен `диалог-подольск.рф`. В nginx нельзя писать кириллицу напрямую — нужно сконвертировать в **punycode**:

```python
>>> "диалог-подольск.рф".encode("idna").decode()
'xn----7sbjcdused0acbk6a6m.xn--p1ai'
```

В `.env`:

```bash
DOMAIN=xn----7sbjcdused0acbk6a6m.xn--p1ai
