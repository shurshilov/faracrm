...
export function ViewWrapper({
  ...
  // контролы (меню «⋮»: колонки, Excel). Пусто для канбана/гантта/формы —
  // там в слот никто не кладёт, поэтому и меню нет.
  const [headerSlot, setHeaderSlot] = useState<HTMLElement | null>(null);

  // Мобильная вёрстка: поиск занимает всю первую строку и всегда открыт, а
  // контролы вида (переключатель + меню «⋮») переносятся на вторую строку
        ...
        <div className={classes.header}>
          <Group justify="space-between" gap="xs" p="xs" wrap="wrap">
            ...
            <Group
              ...
              {/* Слот активного вида: список кладёт сюда своё меню «⋮»
                  (колонки, Excel) через портал (см. HeaderSlotContext).
                  Последним в ряду — как принято для меню «ещё». */}
